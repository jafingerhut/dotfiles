...
;;      maintenance.
;;
;; "cscope-use-quick-inverted-index"
;;      If non-nil, use '-q' option to build extra stuff in cscope
;;      database for quicker searching, and take advantage of this
;;      extra stuff while searching.  Setting this to non-nil speeds
;;      up searches, at the cost of making database creation take
;;      longer.
;;
...
(defcustom cscope-use-quick-inverted-index t
  "*If non-nil, use '-q' option to cscope to build extra stuff in
database for quicker searching, at the cost of extra database build
time."
  :type 'boolean
  :group 'cscope)
...
searching.")
(make-variable-buffer-local 'cscope-sentinel-func)
...
(easy-menu-define cscope:menu
		  ...
		  '("Cscope"
			...
			:style toggle :selected cscope-do-not-update-database ]
		      [ "Build extra stuff in cscope database to speed up searches"
			(setq cscope-use-quick-inverted-index
			      (not cscope-use-quick-inverted-index))
			:style toggle :selected cscope-use-quick-inverted-index ]
		      [ "Index recursively"
...
(defun cscope-search-one-database ()
  ...
  (let ( next-item options cscope-directory database-file outbuf done
	...
	(if cscope-use-quick-inverted-index
...
;; 	;; Extra debug output
;; 	(insert "cscope-filter-func: " (format "%s" cscope-filter-func) "\n")
;; 	(insert "outbuf: " (format "%s" outbuf) "\n")
;; 	(insert "cscope-program: " (format "%s" cscope-program) "\n")
;; 	(insert "options: " (format "%s" options) "\n")
