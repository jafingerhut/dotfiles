...
# f0 - find non-directory and non-symbol-link files with null char
# as separator, to properly handle file names with white space in
# them.
alias f0='find . \! -type d -a \! -type l -print0'
# f0n - Same as f0, except skip over any directories named .git or
# .snapshot, and any files within them.  For git clones with large
# .git contents, this can significantly speed up commands like this:
#     `f0n | x0 grep <pattern>`
# Directories named .snapshot are auto-generated by some Linux
# auto-snapshotting software that makes older versions of your home
# directory available, as a quick recovery mechanism for deleting or
# modifying files that you soon regret.
